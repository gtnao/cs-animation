...
interface LNode {
  id: number;
  key: number;
  s: number; // s-value (null path length)
  left: number | null;
  right: number | null;
}
...
function createNode(key: number): LNode {
  return { id: nextId++, key, s: 1, left: null, right: null };
}

function cloneNodes(nodes: LNode[]): LNode[] {
  return nodes.map((n) => ({ ...n }));
}

function findNode(nodes: LNode[], id: number | null): LNode | null {
  if (id === null) return null;
  return nodes.find((n) => n.id === id) || null;
}

function sValue(nodes: LNode[], id: number | null): number {
  if (id === null) return 0;
...
function generateSteps(operations: string): Step[] {
  ...
  steps.push({
    ...
    rootId: null,
    highlightIds: [],
    description: "Leftist Heap を初期化",
  });

  // Merge two heaps, recording steps
  function merge(
    h1: number | null,
    h2: number | null
  ): number | null {
    ...
    const sLeft = sValue(nodes, n1.left);
    const sRight = sValue(nodes, n1.right);
    ...
    if (sLeft < sRight) {
      ...
      steps.push({
        ...
        rootId,
        highlightIds: [h1],
        description: `key=${n1.key}: 左右の子を交換 (leftist性質維持)`,
      });
    }

    n1.s = Math.min(sValue(nodes, n1.left), sValue(nodes, n1.right)) + 1;
  ...
  for (const op of ops) {
    if (op.startsWith("i")) {
      ...
      const minNode = findNode(nodes, rootId)!;
      ...
      steps.push({
        ...
        rootId,
        highlightIds: [rootId],
        description: `Delete-Min: 最小ノード(key=${minNode.key})を削除`,
      });

      const leftId = minNode.left;
      const rightId = minNode.right;
...
export default function LeftistHeapAnimationPage() {

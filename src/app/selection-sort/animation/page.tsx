...
type StepType = "init" | "scan" | "new-min" | "swap" | "no-swap" | "done";
...
function generateSteps(arr: number[]): Step[] {
  ...
  steps.push({
    ...
    array: [...a],
    sortedIndices: [],
    description: `配列 [${a.join(", ")}] を選択ソートで整列します`,
  });
  ...
  for (let i = 0; i < n - 1; i++) {
    ...
    for (let j = i + 1; j < n; j++) {
      steps.push({
        ...
        description: `a[${j}] = ${a[j]} を現在の最小値 a[${minIdx}] = ${a[minIdx]} と比較`,
      });
      ...
      if (a[j] < a[minIdx]) {
        ...
        steps.push({
          ...
          scanIdx: j,
          sortedIndices: [...sorted],
          description: `新しい最小値: a[${minIdx}] = ${a[minIdx]}`,
        });
      }
    }
    ...
    if (minIdx !== i) {
      ...
      steps.push({
        ...
        minIdx,
        sortedIndices: [...sorted],
        description: `a[${i}] と a[${minIdx}] を交換 → a[${i}] = ${a[i]}`,
      });
    } else {
      steps.push({
        type: "no-swap",
        ...
        current: i,
        sortedIndices: [...sorted],
        description: `a[${i}] = ${a[i]} は既に最小値なので交換不要`,
      });
    }
...
function getCellClass(idx: number, step: Step): string {
  ...
  if (step.type === "swap" && (idx === step.current || idx === step.minIdx)) {
    return `${base} bg-red-100 border-red-500`;
  }

  if (idx === step.scanIdx && step.type === "scan") {
    return `${base} bg-blue-100 border-blue-400`;
  }

  if (idx === step.minIdx && (step.type === "scan" || step.type === "new-min")) {
    return `${base} bg-amber-50 border-amber-400`;
  }

  if (idx === step.current && step.type !== "done" && step.type !== "init") {
    return `${base} bg-blue-100 border-blue-400`;
  }
...
export default function SelectionSortAnimationPage() {
